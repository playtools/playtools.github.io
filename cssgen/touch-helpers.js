...
function TouchScroller(mainElement, excludedClassNames) {
    if (!TouchScroller.IsInitialized) {
        ...
        if (TouchHelpers.IsSupported) {
            document.addEventListener("touchstart", function(e) {
                if (TouchScroller.Hold) e.preventDefault();
            }, false);
    ...
    if (TouchHelpers.IsSupported) {
        if (_element.addEventListener) {
            _element.addEventListener("touchstart", function(ev) {
                me.OnStart(ev);
            }, true);
            _element.addEventListener("touchmove", function(ev) {
                me.OnMove(ev);
            }, true);
            _element.addEventListener("touchend", function(ev) {
                me.OnEnd(ev);
            }, true);
        } else {
            _element.attachEvent("ontouchstart", function() {
                me.OnStart(window.event);
            });
            _element.attachEvent("ontouchmove", function() {
                me.OnMove(window.event);
            });
            _element.attachEvent("ontouchend", function() {
                me.OnEnd(window.event);
            });
        }
    } else {
        if (_element.addEventListener) {
            ...
            _element.addEventListener("mousemove", function(ev) {
                me.OnMove(ev);
            }, true);
            ...
            if (typeof(document.onmousewheel) == "undefined") {
                if (window.opera)
                    _element.addEventListener("mousewheel", function(ev) {
                        me.OnWheel(ev);
                    }, false);
                else
                    _element.addEventListener("DOMMouseScroll", function(ev) {
                        me.OnWheel(ev);
                    }, false);
            } else {
                _element.addEventListener("mousewheel", function(ev) {
            ...
            _element.attachEvent("onmousedown", function() {
                me.OnStart(window.event);
            }, true);
            _element.attachEvent("onmousemove", function() {
                me.OnMove(window.event);
            }, true);
            _element.attachEvent("onmouseup", function() {
                me.OnEnd(window.event);
            }, true);
            _element.attachEvent("onmousewheel", function() {
                me.OnWheel(window.event);
            }, false);
        }
    }
}
